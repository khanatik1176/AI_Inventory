...
export default function PdfUploader() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-zinc-900 to-black flex items-center justify-center p-6">
      <div className="w-full max-w-6xl bg-white/5 border border-white/10 rounded-2xl shadow-2xl p-6 backdrop-blur-sm">
        ...
        <h2 className="text-lg font-semibold text-slate-200 mb-4">Product Table</h2>

        <div className="overflow-x-auto rounded-lg border border-white/6 bg-white/3 p-2">
          <table className="min-w-full divide-y divide-white/6 text-sm">
            <thead>
              <tr className="text-left text-xs text-slate-300 uppercase">
            ...
            <tbody className="divide-y divide-white/6">
              {products.length === 0 ? (
                <tr>
                  <td colSpan={10} className="px-3 py-4 text-slate-400">No products available</td>
                </tr>
              ) : (
                products.map((p) => (
                  <tr key={p.id} className="hover:bg-white/4 transition">
                    <td className="px-3 py-3">{p.document}</td>
                    <td className="px-3 py-3">{p.product_name}</td>
                    <td className="px-3 py-3 text-slate-300">{p.brand_name}</td>
                    <td className="px-3 py-3">{p.product_type}</td>
                    <td className="px-3 py-3">{p.retail_price}</td>
                    <td className="px-3 py-3">{p.sale_price}</td>
                    <td className="px-3 py-3">{p.model_number}</td>
                    <td className="px-3 py-3">{p.color}</td>
                    <td className="px-3 py-3">{p.variants}</td>
                    <td className="px-3 py-3">{p.vendor_name}</td>
                  </tr>
                ))
              )}
            </tbody>
